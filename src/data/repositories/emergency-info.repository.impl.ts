import {
  EmergencyInfo,
  EmergencyInfoInput,
} from "@domain/entities/emergency-info.entity";
  ...
  async save(emergencyInfoInput: EmergencyInfoInput): Promise<EmergencyInfo> {
    try {
      ...
      const encryptedMedications = await Promise.all(
        emergencyInfoInput.medications.map((med) =>
          EncryptionService.encrypt(med),
        ),
      );
      ...
      await this.storage.set(
        this.EMERGENCY_INFO_KEY,
        JSON.stringify(emergencyInfo),
      );
  ...
  async get(): Promise<EmergencyInfo | null> {
    try {
      ...
      const decryptedMedications = await Promise.all(
        (savedInfo.medications || []).map((med: string) =>
          EncryptionService.decrypt(med),
        ),
      );
