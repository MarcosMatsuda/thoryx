...
export function ActionCard({ 
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
    >
      <View className={`w-16 h-16 md:w-20 md:h-20 rounded-full items-center justify-center mb-2.5 ${getCircleBackground()}`}>
        <Text className="text-2xl md:text-3xl">{icon}</Text>
      </View>
      <Text className={`text-sm md:text-base font-bold text-center ${getTextColor()}`}>
        {label}
      </Text>
    </Pressable>
  );
