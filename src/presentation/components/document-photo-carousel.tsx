...
export function DocumentPhotoCarousel({ frontPhotoUri, backPhotoUri }: DocumentPhotoCarouselProps) {
  ...
  return (
    <View className="mb-6">
      ...
      <View className="flex-row justify-center gap-2 mt-4">
        <View className={`w-2 h-2 rounded-full ${currentIndex === 0 ? 'bg-primary-main' : 'bg-ui-border'}`} />
        <View className={`w-2 h-2 rounded-full ${currentIndex === 1 ? 'bg-primary-main' : 'bg-ui-border'}`} />
      </View>
    </View>
  );
}
