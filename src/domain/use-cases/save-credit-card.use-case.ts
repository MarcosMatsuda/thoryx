...
export class SaveCreditCardUseCase {
  ...
  async execute(cardInput: CreditCardInput): Promise<{ success: boolean; message: string; cardId?: string }> {
    ...
    const expirationValidation = cardValidator.expirationDate(cardInput.expiryDate);
    ...
    const cardholderValidation = cardValidator.cardholderName(cardInput.cardholderName);
