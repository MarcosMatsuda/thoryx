...
export class SaveDocumentUseCase {
  ...
  async execute(documentInput: DocumentInput): Promise<{ success: boolean; message: string; documentId?: string }> {
    if (!documentInput.documentNumber || documentInput.documentNumber.trim() === '') {
      return {
        success: false,
